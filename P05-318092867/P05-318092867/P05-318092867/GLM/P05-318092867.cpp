...
// Declaración de modelos de piezas de Automovil B
Model AutoB_modelo;		// Modelo completo 	
Model AutoB_base;		// base del auto
Model AutoB_cofre;		// cofre
Model AutoB_LLTI;		// Llanta 1
Model AutoB_LLDI;		// Llanta 2
Model AutoB_LLTD;		// Llanta 3
Model AutoB_LLDD;		// Llanta 4
...
//	Goddard_M.LoadModel("Models/goddard_cuerpo_obj.obj");	//exportar en este formato cabeza y mandibula
	...
	// Importar modelos de Auto
	AutoB_modelo = Model();		// Modelo completo
	AutoB_modelo.LoadModel("Models/AutoB_modelo.obj");
	AutoB_base = Model();		
	AutoB_base.LoadModel("Models/AutoB_base.obj");
	AutoB_cofre = Model();
	AutoB_cofre.LoadModel("Models/AutoB_cofre.obj");
	AutoB_LLTI = Model();		// Llanta trasera izquierda
	AutoB_LLTI.LoadModel("Models/AutoB_LLTI.obj");
	AutoB_LLDI = Model();		// Llanta delatera izquierda
	AutoB_LLDI.LoadModel("Models/AutoB_LLDI.obj");
	AutoB_LLTD = Model();		// Llanta trasera derecha
	AutoB_LLTD.LoadModel("Models/AutoB_LLTD.obj");
	AutoB_LLDD = Model();		// Llanta delantera derecha
	AutoB_LLDD.LoadModel("Models/AutoB_LLDD.obj");
...
*/
		...
		//cofre
		modelaux = model;
		model = glm::translate(model, glm::vec3(-0.19f, 2.255f, 7.005f));
		model = glm::rotate(model, 17 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getangulocofre()), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		AutoB_cofre.RenderModel();
